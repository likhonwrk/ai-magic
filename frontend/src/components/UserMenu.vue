<template>
    <div class="pointer-events-auto cursor-default">
        <div class="min-w-max inline-block transition-[transform,opacity,scale] duration-150" tabindex="-1" role="dialog">
            <div class="flex w-[300px] flex-col bg-[var(--background-menu-white)] rounded-[20px] border-[0.5px] border-[var(--border-dark)] shadow-[0px_8px_32px_0px_var(--shadow-XS)]">
                    <div class="flex gap-2 px-4 pt-5 pb-3 w-full">
                        ...
                        <div class="flex overflow-hidden flex-col justify-center">
                            <div class="flex gap-1 items-center w-full"><span
                                    class="text-[var(--text-primary)] text-base font-semibold leading-[22px] truncate">{{ currentUser?.fullname || t('Unknown User') }}</span></div><span
                                class="text-[var(--text-tertiary)] text-[13px] font-normal leading-[18px] truncate">{{ currentUser?.email || t('No email') }}</span>
                        </div>
                    </div>
                    <div class="flex flex-col gap-3 px-3 pb-3">
                        <div class="flex flex-col gap-1">
                            ...
                            <div v-if="authProvider !== 'none'" 
                                ...
                                <span
                                    class="overflow-hidden flex-1 text-sm font-medium leading-5 whitespace-nowrap text-ellipsis">{{ t('Logout') }}</span>
                            </div>
                        </div>
                    </div>
            </div>
